...
import pandas as pd
import numpy as np      # 王
from datafactory.futures import domestic_cons as ct
try:
    ...
    from httplib2 import IncompleteRead
...
def get_czce_daily(date=None, type="future"):
    ...
    try:
        html = urlopen(Request(url % (day.strftime('%Y'),
                                                    day.strftime('%Y%m%d')),
                               headers=ct.SIM_HAEDERS)).read().decode('utf-8', 'ignore')
    ...
    # for i in html.split('\n'):
    #     print(i)
    #     if len(i)>0 and i[0]!=u'小':
    #         print('161',True)
    html = [i.replace(' ','').split('|') for i in html.split('\n') if len(i)>0 and i[0] != u'小']
    # print(html)
    if html[1][0] not in [u'品种月份', u'合约代码']:
            return
    ...
    for row in html[2:]:
        ...
        for i,field in enumerate(listed_columns):
            if row[i+1] == "\r":
                row_dict[field] = 0.0       # 王：如果最后一列为空，则赋0.0。
            elif field in ['volume', 'open_interest', 'oi_chg', 'exercise_volume']:
                row[i+1] = row[i+1].replace(',','')
                row_dict[field] = int(row[i+1]) if row[i+1] not in[''] else np.nan
            else:
                row[i+1] = row[i+1].replace(',','')
                row_dict[field] = float(row[i+1]) if row[i+1] not in[''] else np.nan
        dict_data.append(row_dict)
