...
import pandas as pd
import numpy as np
from datafactory.fund import cons as ct
...
def get_nav_open(fund_type='all'):
    ...
    if ct._check_nav_oft_input(fund_type) is True:
        ct._write_head()
        print('url:',ct.SINA_NAV_COUNT_URL %
        ...
        pages = 2  # 分两次请求数据
        # print(type(nums),type(pages))
        limit_cnt = int(nums/pages)+1   # 每次取的数量
        fund_dfs = pd.DataFrame()       # 郑
        ...
        return pd.concat([fund_dfs,fund_df], ignore_index=True,axis=0)      # 郑
...
def get_nav_history(code, start=None, end=None, retry_count=3, pause=0.001, timeout=10):
    ...
    fund_type = df_fund.iloc[0]['Type2Name']     # 王
...
def _parse_fund_data(url, fund_type='open'):
    ...
    try:
        ...
        # print(302, text)
        text = text.split('"data":')[-1].split(',"exec_time"')[0]       # 郑
        # print(304, text)
        # reg = re.compile(r'\,(.*?)\:')
        # # print(306, reg)
